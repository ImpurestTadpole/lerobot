...
class LeRobotDatasetMetadata:
    ...
    def depth_keys(self) -> list[str]:
        """Keys to access depth-map modalities stored as depth images or as videos.

        A depth key is either a feature with ``dtype == "depth"`` (raw depth images),
        or a video feature whose ``info`` dict carries ``"is_depth_map": True``
        (or the legacy ``"video.is_depth_map"`` inside ``info`` or ``video_info``).
        """

        return [key for key, ft in self.features.items() if ft["dtype"] == "depth" or is_depth_map(ft)]
