...
from lerobot.utils.constants import ACTION_DIM_MASK, PRETRAINED_MODEL_DIR, TRAINING_STATE_DIR
...
def make_dataloaders(
    ...
    if not cfg.dataset.streaming:
        ...
        shuffle = False
        # Both branches share the (seed, epoch)-derived permutation property: every rank
        # independently produces the same order, accelerate shards it disjointly across ranks
        # via BatchSamplerShard without needing a shared `generator`, and resume is sample-exact.
        if cfg.source_sampling is not None:
            # Use WeightedEpisodeAwareSampler instead of EpisodeAwareSampler when source_sampling
            # is configured (see config/generalist_source_weights.yaml), to keep one large
            # co-training source from dominating draw frequency.
