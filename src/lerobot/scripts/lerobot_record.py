...
def record_loop(
    ...
    *timer* lets a caller that runs several phases — :func:`record` records one episode
    ...
    camera_obs_keys += [
        f"{k}_depth" for k, cam in getattr(robot, "cameras", {}).items() if getattr(cam, "use_depth", False)
    ]
    last_camera_obs = {}
    ...
    while timestamp < control_time_s:
        ...
        with timer.section("observe"):
            if robot_supports_depth_throttle:
        ...
        with timer.section("teleop"):
            ...
            if isinstance(teleop, Teleoperator):
                ...
                if getattr(teleop.config, "stream_cameras_to_vr", False) and hasattr(
                    teleop, "send_camera_frames"
                ):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                if isinstance(teleop, XLerobotVRTeleop) and getattr(teleop, "vr_event_handler", None):
                    ...
                    while not events.get("recording_gate_open") and not events["stop_recording"]:
                        ...
                        # LEFT X can never be observed if the headset browser never completed the
                        # WSS handshake in the first place -- most commonly because the operator
                        # only accepted the self-signed cert on the HTTPS static-file port
                        # (8443), not on the separate WSS control port (8442); browsers trust a
                        # self-signed cert per origin+port, not per origin. Without this check,
                        # that failure looks identical from here to "connected but not pressing
                        # X yet" -- an indefinite, silent hang with no clue what to fix.
                        if (
                            not no_client_warned
                            and time.perf_counter() - gate_wait_start > 5.0
                            and getattr(teleop, "vr_monitor", None) is not None
                            and hasattr(teleop.vr_monitor, "has_connected_client")
                            and not teleop.vr_monitor.has_connected_client()
                        ):
                            no_client_warned = True
                            logging.warning(
                                "⚠️  No VR headset has connected to the control server yet — LEFT X "
                                "can't be seen. In the headset browser, open the control-server URL "
                                "printed at startup (https://<ip>:8442) in a separate tab and accept "
                                "the certificate warning there too (accepting it on the :8443 page "
                                "does not also cover :8442)."
                            )
