...
class PI05Policy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        if prefix_mask is None:
            per_dim_loss = losses.mean(dim=(0, 1))
        else:
            postfix_mask = (~prefix_mask).unsqueeze(-1).expand_as(losses)
            per_dim_loss = (losses * postfix_mask).sum(dim=(0, 1)) / postfix_mask.sum(dim=(0, 1)).clamp(min=1)
        loss_dict = {
            # Intentionally unmasked by the co-training dim_mask below (raw per-dim value) even
            # when a dim mask is present — this is what lets validation confirm masking is
            # actually excluding the padded dims (their raw loss stays visible here while not
            # contributing to the training signal). RTC prefix timesteps ARE excluded above since
            # their loss is trivial by construction (clean, unnoised) and would skew this figure.
            "loss_per_dim": per_dim_loss.detach().cpu().numpy().tolist(),
        }
        ...
        # Combine the co-training dim_mask (excludes padded/fill action dims) with the RTC
        # postfix_mask (excludes the clean prefix timesteps) into a single valid-element mask,
        # so `denom` counts only elements that are both a real command dim and not part of the
        # RTC prefix — either mask alone degenerates cleanly to the other's original behavior.
        valid_mask = None
        if prefix_mask is not None:
            valid_mask = (~prefix_mask).unsqueeze(-1).expand_as(losses).to(losses.dtype)
        if dim_mask is not None:
            m = dim_mask[:, None, :original_action_dim].to(losses.dtype).expand_as(losses)
            valid_mask = m if valid_mask is None else valid_mask * m

        if valid_mask is not None:
            losses = losses * valid_mask
            denom = valid_mask.sum(dim=(1, 2)).clamp(min=1)  # [B], real (unmasked, non-prefix) elements
        else:
            denom = None
