...
def run_s2(
    ...
    try:
        _run_s2_inner(checkpoint_path, shared_cache, shared_images, task, config,
                      ...
                      throttle_ms=throttle_ms, inject_active=inject_active, dtype=dtype)
    except Exception:
...
def _run_s2_inner(checkpoint_path, shared_cache, shared_images, task, config,
                  device, image_keys, decode_subtask, norm_stats_path, stop_event,
                  throttle_ms=100, inject_active=None, dtype="bfloat16"):
    # Load model
    _dtype_map = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
    torch_dtype = _dtype_map.get(dtype, torch.bfloat16)
    logger.info("S2: Loading VLM from %s (dtype=%s)...", checkpoint_path, dtype)
    model = S2VLMModel.from_pretrained(checkpoint_path, config)
    model.to(device=device, dtype=torch_dtype)
    model.eval()
    logger.info("S2: VLM loaded on %s dtype=%s", device, dtype)
