...
from lerobot.policies.hvla.ipc import (
    SharedLatentCache, SharedImageBuffer, parse_shm_image_size,
    ZMQLatentPublisher, ZMQImageSubscriber,
)
...
def main():
    ...
    parser.add_argument(
        "--shm-image-size",
        default=None,
        help="Height×Width for shared RGB SHM (default 720×1280, or 360×640 with --hvla-preset xlerobot).",
    )
    parser.add_argument(
        "--hvla-preset",
        choices=["none", "xlerobot"],
        default="none",
        help="xlerobot: default SHM size 360×640 (must match lerobot.policies.hvla.launch).",
    )
    ...
    parser.add_argument(
        ...
        help="Model dtype. Use float16 on GPUs without bfloat16 support or to save VRAM (default: bfloat16).",
    )
    # --- Cross-machine ZMQ mode (GPU machine ↔ Jetson) ---
    parser.add_argument(
        "--zmq-image-host",
        default=None,
        help=(
            "IP of the Jetson running S1 (e.g. 192.168.1.50). "
            "When set, S2 receives camera frames over ZMQ instead of shared memory. "
            "Must match --zmq-image-port used by launch.py on the Jetson."
        ),
    )
    parser.add_argument(
        "--zmq-image-port",
        type=int,
        default=ZMQImageSubscriber.DEFAULT_PORT,
        help="ZMQ port for incoming images from Jetson S1 (default: %(default)s).",
    )
    parser.add_argument(
        "--zmq-latent-port",
        type=int,
        default=ZMQLatentPublisher.DEFAULT_PORT,
        help=(
            "ZMQ port to publish latents on (default: %(default)s). "
            "Jetson S1 must use --zmq-latent-port with the same value."
        ),
    )
    args = parser.parse_args()

    zmq_mode = args.zmq_image_host is not None

    if args.hvla_preset == "xlerobot" and args.shm_image_size is None:
    ...
    if zmq_mode:
        # Cross-machine: receive images from Jetson, publish latent to Jetson
        logger.info(
            "ZMQ mode: subscribing to images from %s:%d, publishing latents on port %d",
            args.zmq_image_host, args.zmq_image_port, args.zmq_latent_port,
        )
        shared_images = ZMQImageSubscriber(
            host=args.zmq_image_host,
            port=args.zmq_image_port,
            camera_keys=s2_image_keys,
        )
        shared_cache = ZMQLatentPublisher(
            latent_dim=2048,
            port=args.zmq_latent_port,
        )
        logger.info("ZMQ S2 ready. Waiting for image frames from Jetson...")
    else:
        # Same-machine: use shared memory (original behaviour)
        ...
        logger.info("S2 SHM image layout: %d×%d (preset=%s)", shm_h, shm_w, args.hvla_preset)
