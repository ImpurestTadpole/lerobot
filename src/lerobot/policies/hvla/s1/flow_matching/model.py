...
class FlowMatchingS1Policy(nn.Module):
    ...
    def from_pretrained(cls, checkpoint_path: str, config: FlowMatchingS1Config | None = None):
        ...
        flat format (model.safetensors in checkpoint dir). Prefer passing the
        checkpoint directory (e.g. ``.../checkpoints/last`` or ``.../checkpoint-50000``).
        """
        import json
        ...
        if path.is_dir():
            ...
            # User may pass .../checkpoint-STEP/model.safetensors but training saves under pretrained_model/
            standard_weights = path.parent / "pretrained_model" / "model.safetensors"
            if path.exists():
                model_file = path
                norm_dir = path.parent
            elif standard_weights.exists():
                model_file = standard_weights
                norm_dir = standard_weights.parent
            else:
                model_file = path
                norm_dir = path.parent
