...
def train(args):
    ...
    # Override action/state dims if provided (e.g. 18-DOF XLeRobot vs 14-DOF default)
    ...
    # Override image keys if provided (e.g. "head,left_wrist,right_wrist" for XLeRobot)
    img_size = 224
    if args.resize_images:
        h, w = [int(x) for x in args.resize_images.split("x")]
        img_size = h
    if args.image_keys:
        keys = [k.strip() for k in args.image_keys.split(",")]
        # Prepend observation.images. prefix if bare key given
        keys = [k if k.startswith("observation.") else f"observation.images.{k}" for k in keys]
        config.image_features = {k: img_size for k in keys}
    elif args.resize_images:
        config.image_features = {k: img_size for k in config.image_features}

    logger.info(
        "Config: chunk=%d, action_dim=%d, state_dim=%d, hidden=%d, dec_layers=%d, "
        "rtc_max_delay=%d, rtc_drop=%.2f, denoise_steps=%d | images: %s",
        config.chunk_size, config.action_dim, config.state_dim, config.hidden_dim,
        config.num_decoder_layers, config.rtc_max_delay, config.rtc_drop_prob,
        config.num_inference_steps, list(config.image_features.keys()),
    )
...
def main():
    ...
    parser.add_argument("--image-keys", type=str, default=None,
                        help="Comma-separated camera keys (bare or full). E.g. 'head,left_wrist,right_wrist' "
                             "or 'observation.images.head,...'. Overrides config.image_features defaults.")
    parser.add_argument("--action-dim", type=int, default=None,
                        help="Action dimensionality. Default 14 (7-DOF × 2 arms). Use 18 for XLeRobot.")
    parser.add_argument("--state-dim", type=int, default=None,
                        help="State dimensionality. Default 14 (7-DOF × 2 arms). Use 18 for XLeRobot.")
