...
class BiGemFollower(Robot):
    """Bimanual GEM follower with one Feetech bus and two ODrive buses."""
    ...
    def __init__(self, config: BiGemFollowerConfig):
        ...
        if len(ids) != len(set(ids)):
            raise ValueError(f"BiGEM Feetech motor IDs must be unique on one bus. Got: {ids}")
        ...
        self._odrive_sides = {
            "left": _ODriveSide(
                "left", self.left_odrive_bus, "left_joint_1", threading.Lock(), threading.Event()
            ),
            "right": _ODriveSide(
                "right", self.right_odrive_bus, "right_joint_1", threading.Lock(), threading.Event()
            ),
        }
    ...
    def _split_calibration(
        ...
        feetech = {k: v for k, v in self.calibration.items() if k in feetech_n/ames}
    ...
    def calibrate(self) -> None:
        ...
        if neck_motors:
            input("\nCalibration: Set BiGEM neck neutral position and press ENTER...")
    ...
    def configure(self) -> None:
        ...
        self._read_odrive_blocking("left")
        self._read_odrive_blocking("right")
        for side in self._odrive_sides.values():
            side.pending_target = None
    ...
    def _read_joint_positions(self) -> dict[str, float]:
        ...
        positions["left_joint_1"] = self._get_odrive_position("left")
        positions["right_joint_1"] = self._get_odrive_position("right")
        return positions

    def _read_odrive_blocking(self, side_name: str) -> float:
        side = self._odrive_sides[side_name]
        pos = float(side.bus.read("Present_Position", side.joint))
        with side.lock:
            side.cached_pos = pos
        return pos
    ...
    def _start_odrive_polling(self) -> None:
        ...
        for side in self._odrive_sides.values():
            ...
            side.stop_event.clear()
            thread = threading.Thread(
                target=self._poll_odrive_side,
                args=(side, 1.0 / hz),
                name=f"{self.id}_{side.name}_odrive_poll",
                daemon=True,
            )
            side.thread = thread
            thread.start()

    def _poll_odrive_side(self, side: _ODriveSide, period: float) -> None:
        next_tick = time.perf_counter()
        while not side.stop_event.is_set():
            try:
                with side.lock:
                    target = side.pending_target
                    side.pending_target = None
                if target is not None:
                    side.bus.sync_write("Goal_Position", {side.joint: target})
                self._read_odrive_blocking(side.name)
            except Exception:
                logger.exception("%s %s ODrive poll error", self, side.name)
            next_tick += period
            sleep = next_tick - time.perf_counter()
            if sleep > 0:
                side.stop_event.wait(sleep)
            else:
                next_tick = time.perf_counter()
    ...
    def _get_odrive_position(self, side_name: str) -> float:
        side = self._odrive_sides[side_name]
        if side.thread is None or side.cached_pos is None:
            return self._read_odrive_blocking(side.name)
        with side.lock:
            return float(side.cached_pos)
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        for side in self._odrive_sides.values():
            if side.joint not in motor_goal_pos:
                continue
            if side.thread is None:
                side.bus.sync_write("Goal_Position", {side.joint: motor_goal_pos[side.joint]})
            else:
                with side.lock:
                    side.pending_target = motor_goal_pos[side.joint]
